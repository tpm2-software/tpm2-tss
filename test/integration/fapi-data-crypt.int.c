...
#define chknull(X)                                                                                 \
    if (!(X)) {                                                                                    \
        LOG_ERROR(str(X) "should not be null");                                                    \
...
static TSS2_RC
signatureCallback(char const     *objectPath,
    ...
    if (1 != EVP_DigestSignInit(mdctx, &pctx, ossl_hash, NULL, priv_key)) {
        goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "OSSL sign init.", error_cleanup);
    }
    if (EVP_PKEY_base_id(priv_key) == EVP_PKEY_RSA) {
        ...
        if (1 != EVP_PKEY_CTX_set_rsa_padding(pctx, signing_scheme)) {
            goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "OSSL set RSA padding.", error_cleanup);
        }
    }
    if (1 != EVP_DigestSignUpdate(mdctx, dataToSign, dataToSignSize)) {
        goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "OSSL sign update.", error_cleanup);
    }
    if (1 != EVP_DigestSignFinal(mdctx, NULL, signatureSize)) {
        goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "OSSL sign final.", error_cleanup);
    }
...
test_fapi_data_crypt(FAPI_CONTEXT *context) {
...
#ifdef PERSISTENT
    r = Fapi_CreateKey(context, "HS/SRK/myRsaCryptKey", "decrypt,0x81000004", policy_name, NULL);
#else
    r = Fapi_CreateKey(context, "HS/SRK/myRsaCryptKey", "decrypt", policy_name, NULL);
#endif
    ...
    r = Fapi_Encrypt(context, "HS/SRK/myRsaCryptKey", &plainText[0], SIZE, &cipherText,
                     &cipherTextSize);
    ...
    r = Fapi_Decrypt(context, "HS/SRK/myRsaCryptKey", cipherText, cipherTextSize, &plainText2,
                     &plainText2_size);
    goto_if_error(r, "Error Fapi_Encrypt", error);

    if (plainText2_size != SIZE || memcmp(plainText, plainText2, plainText2_size) != 0) {
    ...
    r = Fapi_Encrypt(context, "/ext/myRsaCryptKey", &plainText[0], SIZE, &cipherText,
                     &cipherTextSize);
