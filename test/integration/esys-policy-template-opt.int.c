...
test_esys_policy_template_opt(ESYS_CONTEXT *esys_context) {
    ...
    r = Esys_PolicyTemplate(esys_context, sessionTrial, ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                            &templateHash);
    if ((r == TPM2_RC_COMMAND_CODE) || (r == (TPM2_RC_COMMAND_CODE | TSS2_RESMGR_RC_LAYER))
        ...
        TPM2B_DIGEST expectedPolicyTemplate
            = { .size = 32,
                .buffer = { 0x70, 0xcb, 0xc9, 0x90, 0x65, 0x3d, 0x8b, 0x40, 0xfb, 0x71, 0xe6,
                            0x77, 0xdf, 0xe5, 0xc8, 0xcb, 0x8b, 0xf5, 0xf4, 0xef, 0xfd, 0x3d,
                            0xed, 0xad, 0x4a, 0x3a, 0xd6, 0xc3, 0x32, 0xc8, 0x35, 0x61 } };

        if (!cmp_policy_digest(esys_context, &sessionTrial, &expectedPolicyTemplate, "Template",
                               FLUSH))
            goto error;
    }
