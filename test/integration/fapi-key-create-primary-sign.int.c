...
#include "tss2_common.h"     // for BYTE, TSS2_RC, TSS2_FAPI_RC_BAD_VALUE
...
test_fapi_key_create_sign(FAPI_CONTEXT *context) {
...
#ifdef PERSISTENT
    r = Fapi_CreateKey(context, "HS/mySignKey", SIGN_TEMPLATE ",0x81000004", "", PASSWORD);
#else
    r = Fapi_CreateKey(context, "HS/mySignKey", SIGN_TEMPLATE "", "", PASSWORD);
#endif
    ...
    r = Fapi_GetTpmBlobs(context, "HS/mySignKey", &publicblob, &publicsize, &privateblob,
                         &privatesize, &policy);
    ...
    r = Fapi_Sign(context, "HS/mySignKey", sigscheme, &digest.buffer[0], digest.size, &signature,
    ...
    r = Fapi_VerifySignature(context, "HS/mySignKey", &digest.buffer[0], digest.size, signature,
                             signatureSize);
