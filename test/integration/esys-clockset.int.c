...
test_esys_clockset(ESYS_CONTEXT *esys_context) {
    ...
    r = Esys_ReadClock(esys_context, session, ESYS_TR_NONE, ESYS_TR_NONE, &currentTime);

    /* TPMs before Revision 1.38 might not support session usage*/
    if ((r == TPM2_RC_AUTH_CONTEXT) || (r == (TPM2_RC_AUTH_CONTEXT | TSS2_RESMGR_RC_LAYER))
    ...
    r = Esys_ClockRateAdjust(esys_context, auth_handle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                             ESYS_TR_NONE, TPM2_CLOCK_MEDIUM_FASTER);
    goto_if_error(r, "Error: ClockRateAdjust", error);

    r = Esys_ClockRateAdjust(esys_context, auth_handle, ESYS_TR_PASSWORD, ESYS_TR_NONE,
                             ESYS_TR_NONE, TPM2_CLOCK_MEDIUM_SLOWER);
    goto_if_error(r, "Error: ClockRateAdjust", error);
