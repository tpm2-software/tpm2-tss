...
test_invoke_esys(ESYS_CONTEXT *esys_context) {
    ...
    CHECK_BACKEND_FN_NOT_TEST(esys_context->crypto_backend, mlkem_encapsulate);

    ESYS_CRYPTO_CALLBACKS callbacks = { .aes_decrypt = TEST_FN_PTR,
    ...
    TSS2_RC r = Esys_SetCryptoCallbacks(esys_context, &callbacks);
    ...
    /* reset state */
    r = Esys_SetCryptoCallbacks(esys_context, NULL);
    if (r != TSS2_RC_SUCCESS) {
        LOG_ERROR("Esys_SetCryptoCallbacks failed: 0x%x", r);
        return EXIT_FAILURE;
    }
