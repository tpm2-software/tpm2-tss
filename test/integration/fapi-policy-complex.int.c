...
auth_callback(char const *objectPath, char const *description, const char **auth, void *userData) {
    ...
    else if (strcmp(objectPath, "/SRK/key-1") == 0)
        *auth = KEY_PASSWORD1;
    else if (strcmp(objectPath, "/SRK/key-2") == 0)
        *auth = KEY_PASSWORD2;
    else if (strcmp(objectPath, "/SRK/key-3") == 0)
        *auth = KEY_PASSWORD3;
    else
        *auth = KEY_PASSWORD4;
    cb_auth_called = true;
...
/** Test the FAPI for PolicyOr and PolicyAuthorizeNv in each branch, with nested PolicyOr and PolicySecret as the authorized policies.
 *
