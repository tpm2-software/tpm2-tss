...
test_fapi_ecc_encrypt_decrypt(FAPI_CONTEXT *context)
{
    ...
    r = Fapi_CreateKey(context, "HS/SRK/myEccCryptKey", "decrypt",
                       NULL, NULL);
    goto_if_error(r, "Error Fapi_CreateKey", error);

    r = Fapi_Encrypt(context, "HS/SRK/myEccCryptKey",
                     &plain[0],
                     SIZE,
                     &cipher,
                     &cipher_size);
    ...
    r = Fapi_Decrypt(context, "HS/SRK/myEccCryptKey",
                     cipher,
                     cipher_size,
                     &plain_out,
                     &plain_out_size);
    ...
    if (plain_out_size != SIZE ||
        memcmp(plain_out, plain, SIZE) != 0) {
