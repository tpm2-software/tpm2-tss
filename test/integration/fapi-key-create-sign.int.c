...
test_fapi_key_create_sign(FAPI_CONTEXT *context) {
...
#ifdef PERSISTENT
    r = Fapi_CreateKey(context, "HS/SRK/mySignKey", SIGN_TEMPLATE ",0x81000004", "", PASSWORD);
#else
    ...
    r = Fapi_GetTpmBlobs(context, "HS/SRK/mySignKey", &publicblob, &publicsize, &privateblob,
                         &privatesize, &policy);
    ...
    r = Fapi_Sign(context, "HS/SRK/mySignKey", sigscheme, &digest.buffer[0], digest.size,
                  &signature, &signatureSize, &publicKey, &certificate);
    ...
    r = Fapi_VerifySignature(context, "HS/SRK/mySignKey", &digest.buffer[0], digest.size, signature,
                             signatureSize);
    ...
    r = Fapi_VerifySignature(context, "HS/SRK/mySignKey2", &digest.buffer[0], digest.size,
