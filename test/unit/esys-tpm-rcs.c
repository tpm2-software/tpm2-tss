...
static TSS2_RC
tcti_tpmerror_transmit(TSS2_TCTI_CONTEXT *tctiContext, size_t size, const uint8_t *buffer) {
    UNUSED(tctiContext);
    UNUSED(size);
    UNUSED(buffer);
...
test_AC_GetCapability(void **state) {
    ...
    TPMI_YES_NO           moreData;
    r = Esys_AC_GetCapability(esys_context, ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE, capability,
                              property, propertyCount, &moreData, &capabilityData);
