...
static TSS2_RC
tcti_failure_transmit(TSS2_TCTI_CONTEXT *tctiContext, size_t size, const uint8_t *buffer) {
    UNUSED(tctiContext);
    UNUSED(size);
    UNUSED(buffer);
...
esys_unit_setup(void **state) {
    ...
    TSS2_TCTI_CONTEXT_COMMON_V1 *tcti = calloc(1, sizeof(TSS2_TCTI_CONTEXT_COMMON_V1));
    tcti->version = 1;
...
check_StartAuthSession(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_StartAuthSession_Finish(esys_context, &sessionHandle_handle);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
check_Create(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_Create_Finish(esys_context, &outPrivate, &outPublic, &creationData, &creationHash,
                               &creationTicket);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
check_ReadPublic(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_ReadPublic_Finish(esys_context, &outPublic, &name, &qualifiedName);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
check_CreateLoaded(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_CreateLoaded_Finish(esys_context, &objectHandle_handle, &outPrivate, &outPublic);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
check_Duplicate(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_Duplicate_Finish(esys_context, &encryptionKeyOut, &duplicate, &outSymSeed);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
check_GetSessionAuditDigest(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_GetSessionAuditDigest_Finish(esys_context, &auditInfo, &signature);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
check_GetCommandAuditDigest(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_GetCommandAuditDigest_Finish(esys_context, &auditInfo, &signature);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
check_PCR_Read(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_PCR_Read_Finish(esys_context, &pcrUpdateCounter, &pcrSelectionOut, &pcrValues);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
check_PCR_Allocate(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_PCR_Allocate_Finish(esys_context, &allocationSuccess, &maxPCR, &sizeNeeded,
                                     &sizeAvailable);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
check_CreatePrimary(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_CreatePrimary_Finish(esys_context, &objectHandle_handle, &outPublic, &creationData,
...
check_FieldUpgradeData(void **state) {
    ...
    for (size_t i = 0; i < sizeof(esys_states) / sizeof(esys_states[0]); i++) {
        esys_context->state = esys_states[i];
        r = Esys_FieldUpgradeData_Finish(esys_context, &nextDigest, &firstDigest);
        assert_int_equal(r, TSS2_ESYS_RC_BAD_SEQUENCE);
    }
}
...
main(void) {
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(check_Startup, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Shutdown, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_SelfTest, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_IncrementalSelfTest, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_GetTestResult, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_StartAuthSession, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyRestart, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Create, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Load, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_LoadExternal, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ReadPublic, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ActivateCredential, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_MakeCredential, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Unseal, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ObjectChangeAuth, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_CreateLoaded, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Duplicate, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Rewrap, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Import, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_RSA_Encrypt, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_RSA_Decrypt, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ECDH_KeyGen, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ECDH_ZGen, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ECC_Parameters, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ZGen_2Phase, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_EncryptDecrypt, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_EncryptDecrypt2, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Hash, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_HMAC, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_GetRandom, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_StirRandom, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_HMAC_Start, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_HashSequenceStart, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_SequenceUpdate, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_SequenceComplete, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_EventSequenceComplete, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Certify, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_CertifyCreation, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Quote, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_GetSessionAuditDigest, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_GetCommandAuditDigest, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_GetTime, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Commit, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_EC_Ephemeral, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_VerifySignature, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Sign, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_SetCommandCodeAuditStatus, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PCR_Extend, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PCR_Event, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PCR_Read, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PCR_Allocate, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PCR_SetAuthPolicy, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PCR_SetAuthValue, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PCR_Reset, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicySigned, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicySecret, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyTicket, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyOR, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyPCR, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyLocality, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyNV, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyCounterTimer, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyCommandCode, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyPhysicalPresence, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyCpHash, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyNameHash, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyDuplicationSelect, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyAuthorize, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyAuthValue, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyPassword, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyGetDigest, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyNvWritten, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyTemplate, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PolicyAuthorizeNV, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_CreatePrimary, esys_unit_setup, esys_unit_teardown),
        ...
        cmocka_unit_test_setup_teardown(check_SetPrimaryPolicy, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ChangePPS, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ChangeEPS, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Clear, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ClearControl, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_HierarchyChangeAuth, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_DictionaryAttackLockReset, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_DictionaryAttackParameters, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_PP_Commands, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_SetAlgorithmSet, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_FieldUpgradeStart, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_FieldUpgradeData, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_FirmwareRead, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ContextSave, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ContextLoad, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_FlushContext, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_EvictControl, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ReadClock, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ClockSet, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_ClockRateAdjust, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_GetCapability, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_TestParms, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_DefineSpace, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_UndefineSpace, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_UndefineSpaceSpecial, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_ReadPublic, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_Write, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_Increment, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_Extend, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_SetBits, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_WriteLock, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_GlobalWriteLock, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_Read, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_ReadLock, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_ChangeAuth, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_NV_Certify, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_Vendor_TCG_Test, esys_unit_setup, esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_AC_GetCapability, esys_unit_setup,
                                        esys_unit_teardown),
        cmocka_unit_test_setup_teardown(check_AC_Send, esys_unit_setup, esys_unit_teardown),
