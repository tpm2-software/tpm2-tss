...
#include "tss2_tpm2_types.h"      // for TPM2_MLKEM_PARAMETER_SET, TPM2_MLDSA_PARAMETER_SET
...
/*
 * TPM2_MLKEM_PARAMETER_SET and TPM2_MLDSA_PARAMETER_SET are both UINT16
 * typedef'd types with dedicated MU functions generated by BASE_MARSHAL.
 */
static void
mlkem_parameter_set_marshal_unmarshal(void **state) {
    TPM2_MLKEM_PARAMETER_SET src = TPM2_MLKEM_768;
    TPM2_MLKEM_PARAMETER_SET dest = TPM2_MLKEM_NONE;
    uint8_t buf[2];
    size_t offset = 0;
    TSS2_RC rc;

    rc = Tss2_MU_TPM2_MLKEM_PARAMETER_SET_Marshal(src, buf, sizeof(buf), &offset);
    assert_int_equal(rc, TSS2_RC_SUCCESS);
    assert_int_equal(offset, 2u);
    assert_int_equal(buf[0], 0x00);
    assert_int_equal(buf[1], 0x02); /* TPM2_MLKEM_768 = 2 */

    offset = 0;
    rc = Tss2_MU_TPM2_MLKEM_PARAMETER_SET_Unmarshal(buf, sizeof(buf), &offset, &dest);
    assert_int_equal(rc, TSS2_RC_SUCCESS);
    assert_int_equal(dest, TPM2_MLKEM_768);
}

static void
mldsa_parameter_set_marshal_unmarshal(void **state) {
    TPM2_MLDSA_PARAMETER_SET src = TPM2_MLDSA_65;
    TPM2_MLDSA_PARAMETER_SET dest = TPM2_MLDSA_NONE;
    uint8_t buf[2];
    size_t offset = 0;
    TSS2_RC rc;

    rc = Tss2_MU_TPM2_MLDSA_PARAMETER_SET_Marshal(src, buf, sizeof(buf), &offset);
    assert_int_equal(rc, TSS2_RC_SUCCESS);
    assert_int_equal(offset, 2u);
    assert_int_equal(buf[0], 0x00);
    assert_int_equal(buf[1], 0x02); /* TPM2_MLDSA_65 = 2 */

    offset = 0;
    rc = Tss2_MU_TPM2_MLDSA_PARAMETER_SET_Unmarshal(buf, sizeof(buf), &offset, &dest);
    assert_int_equal(rc, TSS2_RC_SUCCESS);
    assert_int_equal(dest, TPM2_MLDSA_65);
}
...
main(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test(mlkem_parameter_set_marshal_unmarshal),
        cmocka_unit_test(mldsa_parameter_set_marshal_unmarshal),
    };
