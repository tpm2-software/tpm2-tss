...
#include "tss2_tpm2_types.h"      // for TPMI_MLKEM_PARMS, TPMI_MLDSA_PARMS
...
/*
 * TPMI_MLKEM_PARMS and TPMI_MLDSA_PARMS are both UINT16
...
mlkem_parameter_set_marshal_unmarshal(void **state) {
    ...
    assert_int_equal(buf[1], 0x02); /* TPM2_MLKEM_PARMS_768 = 2 */

    offset = 0;
    rc = Tss2_MU_TPMI_MLKEM_PARMS_Unmarshal(buf, sizeof(buf), &offset, &dest);
    assert_int_equal(rc, TSS2_RC_SUCCESS);
    assert_int_equal(dest, TPM2_MLKEM_PARMS_768);
}
...
mldsa_parameter_set_marshal_unmarshal(void **state) {
    ...
    assert_int_equal(offset, 2u);
    assert_int_equal(buf[0], 0x00);
    assert_int_equal(buf[1], 0x02); /* TPM2_MLDSA_PARMS_65 = 2 */

    offset = 0;
    rc = Tss2_MU_TPMI_MLDSA_PARMS_Unmarshal(buf, sizeof(buf), &offset, &dest);
    assert_int_equal(rc, TSS2_RC_SUCCESS);
    assert_int_equal(dest, TPM2_MLDSA_PARMS_65);
}
