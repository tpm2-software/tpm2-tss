...
TSS2_RC
platform_spi_transfer_with_wait_state(void       *user_data,
                                      const void *data_out,
                                      void       *data_in,
                                      size_t      cnt) {
    UNUSED(user_data);
...
TSS2_RC
platform_spi_transfer_fuzz(void *user_data, const void *data_out, void *data_in, size_t cnt) {
    UNUSED(data_out);
...
platform_spi_transfer(void *user_data, const void *data_out, void *data_in, size_t cnt) {
     ...
     */
    return is_init ? platform_spi_transfer_with_wait_state(user_data, data_out, data_in, cnt)
...
int
LLVMFuzzerTestOneInput(const uint8_t *fuzz_data, size_t size) {
    if (!fuzz_data) {
        return -1;
    }
    ...
    TSS2_TCTI_SPI_HELPER_PLATFORM conf = { .user_data = &udata,
