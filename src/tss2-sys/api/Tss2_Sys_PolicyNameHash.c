...
TSS2_RC
Tss2_Sys_PolicyNameHash_Prepare(TSS2_SYS_CONTEXT   *sysContext,
    ...
    if (!nameHash) {
        ...
        rval = Tss2_MU_TPM2B_DIGEST_Marshal(nameHash, ctx->cmdBuffer, ctx->maxCmdSize,
                                            &ctx->nextData);
    }
...
TSS2_RC
Tss2_Sys_PolicyNameHash_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_PolicyNameHash(TSS2_SYS_CONTEXT             *sysContext,
                        TPMI_SH_POLICY                policySession,
