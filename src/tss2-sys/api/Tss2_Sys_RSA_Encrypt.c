...
TSS2_RC
Tss2_Sys_RSA_Encrypt_Prepare(TSS2_SYS_CONTEXT           *sysContext,
    ...
    if (!message) {
        ...
        rval = Tss2_MU_TPM2B_PUBLIC_KEY_RSA_Marshal(message, ctx->cmdBuffer, ctx->maxCmdSize,
                                                    &ctx->nextData);
    }
...
TSS2_RC
Tss2_Sys_RSA_Encrypt_Complete(TSS2_SYS_CONTEXT *sysContext, TPM2B_PUBLIC_KEY_RSA *outData) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
...
TSS2_RC
Tss2_Sys_RSA_Encrypt(TSS2_SYS_CONTEXT             *sysContext,
                     TPMI_DH_OBJECT                keyHandle,
