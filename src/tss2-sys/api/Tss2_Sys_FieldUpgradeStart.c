...
TSS2_RC
Tss2_Sys_FieldUpgradeStart_Prepare(TSS2_SYS_CONTEXT     *sysContext,
    ...
    if (rval)
        return rval;
    rval = Tss2_MU_UINT32_Marshal(authorization, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
    ...
    if (!fuDigest) {
        ...
        rval = Tss2_MU_TPM2B_DIGEST_Marshal(fuDigest, ctx->cmdBuffer, ctx->maxCmdSize,
                                            &ctx->nextData);
    }
    ...
    rval = Tss2_MU_TPMT_SIGNATURE_Marshal(manifestSignature, ctx->cmdBuffer, ctx->maxCmdSize,
                                          &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_FieldUpgradeStart_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_FieldUpgradeStart(TSS2_SYS_CONTEXT             *sysContext,
                           TPMI_RH_PLATFORM              authorization,
                           TPMI_DH_OBJECT                keyHandle,
    ...
    rval = Tss2_Sys_FieldUpgradeStart_Prepare(sysContext, authorization, keyHandle, fuDigest,
                                              manifestSignature);
    if (rval)
        return rval;
