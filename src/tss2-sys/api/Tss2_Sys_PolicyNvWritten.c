...
TSS2_RC
Tss2_Sys_PolicyNvWritten_Prepare(TSS2_SYS_CONTEXT *sysContext,
    ...
    rval = Tss2_MU_UINT8_Marshal(writtenSet, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_PolicyNvWritten_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_PolicyNvWritten(TSS2_SYS_CONTEXT             *sysContext,
                         TPMI_SH_POLICY                policySession,
