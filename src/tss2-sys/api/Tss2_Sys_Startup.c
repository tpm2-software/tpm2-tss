...
Tss2_Sys_Startup_Prepare(TSS2_SYS_CONTEXT *sysContext, TPM2_SU startupType) {
    ...
    rval = Tss2_MU_UINT16_Marshal(startupType, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_Startup_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_Startup(TSS2_SYS_CONTEXT *sysContext, TPM2_SU startupType) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
