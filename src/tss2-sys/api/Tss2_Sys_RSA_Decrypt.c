...
TSS2_RC
Tss2_Sys_RSA_Decrypt_Prepare(TSS2_SYS_CONTEXT           *sysContext,
    ...
    if (!cipherText) {
        ...
        rval = Tss2_MU_TPM2B_PUBLIC_KEY_RSA_Marshal(cipherText, ctx->cmdBuffer, ctx->maxCmdSize,
                                                    &ctx->nextData);
    }
    ...
    rval = Tss2_MU_TPMT_RSA_DECRYPT_Marshal(inScheme, ctx->cmdBuffer, ctx->maxCmdSize,
                                            &ctx->nextData);
    if (rval)
        return rval;
    ...
    if (!label) {
        ...
        rval = Tss2_MU_TPM2B_DATA_Marshal(label, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    }
...
TSS2_RC
Tss2_Sys_RSA_Decrypt_Complete(TSS2_SYS_CONTEXT *sysContext, TPM2B_PUBLIC_KEY_RSA *message) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
    ...
    return Tss2_MU_TPM2B_PUBLIC_KEY_RSA_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData,
                                                  message);
}

TSS2_RC
Tss2_Sys_RSA_Decrypt(TSS2_SYS_CONTEXT             *sysContext,
                     TPMI_DH_OBJECT                keyHandle,
