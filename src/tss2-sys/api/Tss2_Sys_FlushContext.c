...
TSS2_RC
Tss2_Sys_FlushContext_Prepare(TSS2_SYS_CONTEXT *sysContext, TPMI_DH_CONTEXT flushHandle) {
    ...
    if (rval)
        return rval;
    rval = Tss2_MU_UINT32_Marshal(flushHandle, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_FlushContext_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_FlushContext(TSS2_SYS_CONTEXT *sysContext, TPMI_DH_CONTEXT flushHandle) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
