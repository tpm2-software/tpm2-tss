...
TSS2_RC
Tss2_Sys_PolicyTemplate_Prepare(TSS2_SYS_CONTEXT   *sysContext,
    ...
    if (!templateHash) {
        ...
        rval = Tss2_MU_TPM2B_DIGEST_Marshal(templateHash, ctx->cmdBuffer, ctx->maxCmdSize,
                                            &ctx->nextData);
    }
...
TSS2_RC
Tss2_Sys_PolicyTemplate_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_PolicyTemplate(TSS2_SYS_CONTEXT             *sysContext,
                        TPMI_SH_POLICY                policySession,
    ...
    rval = Tss2_Sys_PolicyTemplate_Prepare(sysContext, policySession, templateHash);
    if (rval)
        return rval;
