...
TSS2_RC
Tss2_Sys_SequenceComplete_Prepare(TSS2_SYS_CONTEXT       *sysContext,
...
TSS2_RC
Tss2_Sys_SequenceComplete_Complete(TSS2_SYS_CONTEXT  *sysContext,
                                   TPM2B_DIGEST      *result,
    ...
    rval = Tss2_MU_TPM2B_DIGEST_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData, result);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_SequenceComplete(TSS2_SYS_CONTEXT             *sysContext,
                          TPMI_DH_OBJECT                sequenceHandle,
