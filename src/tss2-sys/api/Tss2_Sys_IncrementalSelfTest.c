...
TSS2_RC
Tss2_Sys_IncrementalSelfTest_Prepare(TSS2_SYS_CONTEXT *sysContext, const TPML_ALG *toTest) {
    ...
    rval = Tss2_MU_TPML_ALG_Marshal(toTest, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_IncrementalSelfTest_Complete(TSS2_SYS_CONTEXT *sysContext, TPML_ALG *toDoList) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
    ...
    return Tss2_MU_TPML_ALG_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData, toDoList);
}

TSS2_RC
Tss2_Sys_IncrementalSelfTest(TSS2_SYS_CONTEXT             *sysContext,
