...
TSS2_RC
Tss2_Sys_Quote_Prepare(TSS2_SYS_CONTEXT         *sysContext,
    ...
    rval = Tss2_MU_TPML_PCR_SELECTION_Marshal(PCRselect, ctx->cmdBuffer, ctx->maxCmdSize,
                                              &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_Quote_Complete(TSS2_SYS_CONTEXT *sysContext,
                        TPM2B_ATTEST     *quoted,
    ...
    rval = Tss2_MU_TPM2B_ATTEST_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData, quoted);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_Quote(TSS2_SYS_CONTEXT             *sysContext,
               TPMI_DH_OBJECT                signHandle,
