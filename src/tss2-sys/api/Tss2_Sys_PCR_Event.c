...
TSS2_RC
Tss2_Sys_PCR_Event_Prepare(TSS2_SYS_CONTEXT  *sysContext,
    ...
    if (!eventData) {
        ...
        rval = Tss2_MU_TPM2B_EVENT_Marshal(eventData, ctx->cmdBuffer, ctx->maxCmdSize,
                                           &ctx->nextData);
    }
...
TSS2_RC
Tss2_Sys_PCR_Event_Complete(TSS2_SYS_CONTEXT *sysContext, TPML_DIGEST_VALUES *digests) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
    ...
    return Tss2_MU_TPML_DIGEST_VALUES_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData,
                                                digests);
}

TSS2_RC
Tss2_Sys_PCR_Event(TSS2_SYS_CONTEXT             *sysContext,
                   TPMI_DH_PCR                   pcrHandle,
