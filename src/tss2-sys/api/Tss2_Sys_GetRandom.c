...
TSS2_RC
Tss2_Sys_GetRandom_Prepare(TSS2_SYS_CONTEXT *sysContext, UINT16 bytesRequested) {
    ...
    if (rval)
        return rval;
    rval = Tss2_MU_UINT16_Marshal(bytesRequested, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_GetRandom_Complete(TSS2_SYS_CONTEXT *sysContext, TPM2B_DIGEST *randomBytes) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
...
TSS2_RC
Tss2_Sys_GetRandom(TSS2_SYS_CONTEXT             *sysContext,
