...
TSS2_RC
Tss2_Sys_Rewrap_Prepare(TSS2_SYS_CONTEXT             *sysContext,
    ...
    rval = Tss2_MU_UINT32_Marshal(oldParent, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;

    rval = Tss2_MU_UINT32_Marshal(newParent, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
    ...
    if (!inDuplicate) {
        ...
        rval = Tss2_MU_TPM2B_PRIVATE_Marshal(inDuplicate, ctx->cmdBuffer, ctx->maxCmdSize,
                                             &ctx->nextData);
    }
    ...
    if (!name) {
        ...
        rval = Tss2_MU_TPM2B_NAME_Marshal(name, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    }
...
TSS2_RC
Tss2_Sys_Rewrap_Complete(TSS2_SYS_CONTEXT       *sysContext,
...
TSS2_RC
Tss2_Sys_Rewrap(TSS2_SYS_CONTEXT             *sysContext,
                TPMI_DH_OBJECT                oldParent,
                TPMI_DH_OBJECT                newParent,
