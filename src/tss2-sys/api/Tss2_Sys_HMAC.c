...
TSS2_RC
Tss2_Sys_HMAC_Prepare(TSS2_SYS_CONTEXT       *sysContext,
...
TSS2_RC
Tss2_Sys_HMAC_Complete(TSS2_SYS_CONTEXT *sysContext, TPM2B_DIGEST *outHMAC) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
    ...
    return Tss2_MU_TPM2B_DIGEST_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData, outHMAC);
}

TSS2_RC
Tss2_Sys_HMAC(TSS2_SYS_CONTEXT             *sysContext,
              TPMI_DH_OBJECT                handle,
