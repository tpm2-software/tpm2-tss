    ...
    TPMS_CAPABILITY_DATA *capabilityData)
{
    ...
    rval = Tss2_MU_UINT32_Unmarshal(&ctx->cmdBuffer[ctx->nextData],
    ...
    if (ctx->maxCmdSize - ctx->nextData - sizeof(cap) >
            sizeof(capabilityData->data.vendor.buffer))
    ...
    capabilityData->data.vendor.size = ctx->maxCmdSize - ctx->nextData;
    memcpy(&capabilityData->data.vendor.buffer[0],
           &ctx->cmdBuffer[ctx->nextData],
           capabilityData->data.vendor.size);
    ctx->nextData += capabilityData->data.vendor.size;
