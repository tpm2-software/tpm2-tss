...
TSS2_RC
Tss2_Sys_CreateLoaded_Prepare(TSS2_SYS_CONTEXT             *sysContext,
    ...
    rval
        = Tss2_MU_TPM2B_TEMPLATE_Marshal(inPublic, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_CreateLoaded_Complete(TSS2_SYS_CONTEXT *sysContext,
...
TSS2_RC
Tss2_Sys_CreateLoaded(TSS2_SYS_CONTEXT             *sysContext,
                      TPMI_RH_HIERARCHY             parentHandle,
    ...
    rval = Tss2_Sys_CreateLoaded_Prepare(sysContext, parentHandle, inSensitive, inPublic);
    if (rval)
        return rval;
    ...
    rval = Tss2_Sys_CreateLoaded_Complete(sysContext, objectHandle, outPrivate, outPublic, name);
    return rval;
}
