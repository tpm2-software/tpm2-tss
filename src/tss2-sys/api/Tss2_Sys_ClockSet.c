...
Tss2_Sys_ClockSet_Prepare(TSS2_SYS_CONTEXT *sysContext, TPMI_RH_PROVISION auth, UINT64 newTime) {
    ...
    if (rval)
        return rval;
    rval = Tss2_MU_UINT64_Marshal(newTime, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_ClockSet_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_ClockSet(TSS2_SYS_CONTEXT             *sysContext,
