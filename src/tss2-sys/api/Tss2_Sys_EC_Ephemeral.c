...
TSS2_RC
Tss2_Sys_EC_Ephemeral_Prepare(TSS2_SYS_CONTEXT *sysContext, TPMI_ECC_CURVE curveID) {
...
Tss2_Sys_EC_Ephemeral_Complete(TSS2_SYS_CONTEXT *sysContext, TPM2B_ECC_POINT *Q, UINT16 *counter) {
    ...
    if (rval)
        return rval;
    rval = Tss2_MU_TPM2B_ECC_POINT_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData, Q);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_EC_Ephemeral(TSS2_SYS_CONTEXT             *sysContext,
