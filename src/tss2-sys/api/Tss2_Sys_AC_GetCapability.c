...
TSS2_RC
Tss2_Sys_AC_GetCapability_Prepare(TSS2_SYS_CONTEXT *sysContext,
                                  TPMI_RH_AC        ac,
                                  TPM_AT            capability,
                                  UINT32            count) {
    ...
    rval = Tss2_MU_UINT32_Marshal(count, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_AC_GetCapability_Complete(TSS2_SYS_CONTEXT     *sysContext,
    ...
    return Tss2_MU_TPML_AC_CAPABILITIES_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData,
                                                  capabilityData);
}

TSS2_RC
Tss2_Sys_AC_GetCapability(TSS2_SYS_CONTEXT             *sysContext,
                          TPMI_RH_AC                    ac,
    ...
    return Tss2_Sys_AC_GetCapability_Complete(sysContext, moreData, capabilityData);
}
