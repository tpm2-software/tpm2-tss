...
TSS2_RC
Tss2_Sys_PolicyLocality_Prepare(TSS2_SYS_CONTEXT *sysContext,
    ...
    rval = Tss2_MU_TPMA_LOCALITY_Marshal(locality, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_PolicyLocality_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_PolicyLocality(TSS2_SYS_CONTEXT             *sysContext,
                        TPMI_SH_POLICY                policySession,
