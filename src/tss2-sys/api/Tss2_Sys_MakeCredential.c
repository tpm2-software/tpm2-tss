...
TSS2_RC
Tss2_Sys_MakeCredential_Prepare(TSS2_SYS_CONTEXT   *sysContext,
    ...
    if (!credential) {
        ...
        rval = Tss2_MU_TPM2B_DIGEST_Marshal(credential, ctx->cmdBuffer, ctx->maxCmdSize,
                                            &ctx->nextData);
    }
...
TSS2_RC
Tss2_Sys_MakeCredential_Complete(TSS2_SYS_CONTEXT       *sysContext,
    ...
    rval = Tss2_MU_TPM2B_ID_OBJECT_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData,
                                             credentialBlob);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_MakeCredential(TSS2_SYS_CONTEXT             *sysContext,
                        TPMI_DH_OBJECT                handle,
