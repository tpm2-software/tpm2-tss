...
TSS2_RC
Tss2_Sys_PCR_SetAuthPolicy_Prepare(TSS2_SYS_CONTEXT   *sysContext,
    ...
    if (!authPolicy) {
        ...
        rval = Tss2_MU_TPM2B_DIGEST_Marshal(authPolicy, ctx->cmdBuffer, ctx->maxCmdSize,
                                            &ctx->nextData);
    }
    ...
    rval = Tss2_MU_UINT32_Marshal(pcrNum, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_PCR_SetAuthPolicy_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_PCR_SetAuthPolicy(TSS2_SYS_CONTEXT             *sysContext,
                           TPMI_RH_PLATFORM              authHandle,
