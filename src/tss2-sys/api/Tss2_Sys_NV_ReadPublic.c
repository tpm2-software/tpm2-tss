...
TSS2_RC
Tss2_Sys_NV_ReadPublic_Prepare(TSS2_SYS_CONTEXT *sysContext, TPMI_RH_NV_INDEX nvIndex) {
...
TSS2_RC
Tss2_Sys_NV_ReadPublic_Complete(TSS2_SYS_CONTEXT *sysContext,
    ...
    rval = Tss2_MU_TPM2B_NV_PUBLIC_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData,
                                             nvPublic);
    if (rval)
        return rval;

    return Tss2_MU_TPM2B_NAME_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData, nvName);
}
