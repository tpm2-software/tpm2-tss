...
TSS2_RC
Tss2_Sys_ECDH_ZGen_Prepare(TSS2_SYS_CONTEXT      *sysContext,
    ...
    if (!inPoint) {
        ...
        rval = Tss2_MU_TPM2B_ECC_POINT_Marshal(inPoint, ctx->cmdBuffer, ctx->maxCmdSize,
                                               &ctx->nextData);
    }
...
TSS2_RC
Tss2_Sys_ECDH_ZGen_Complete(TSS2_SYS_CONTEXT *sysContext, TPM2B_ECC_POINT *outPoint) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
...
TSS2_RC
Tss2_Sys_ECDH_ZGen(TSS2_SYS_CONTEXT             *sysContext,
                   TPMI_DH_OBJECT                keyHandle,
