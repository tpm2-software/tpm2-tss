...
TSS2_RC
Tss2_Sys_Hash_Prepare(TSS2_SYS_CONTEXT       *sysContext,
    ...
    if (!data) {
        ...
        rval = Tss2_MU_TPM2B_MAX_BUFFER_Marshal(data, ctx->cmdBuffer, ctx->maxCmdSize,
                                                &ctx->nextData);
    }
...
TSS2_RC
Tss2_Sys_Hash_Complete(TSS2_SYS_CONTEXT  *sysContext,
    ...
    rval = Tss2_MU_TPM2B_DIGEST_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData, outHash);
    if (rval)
        return rval;

    return Tss2_MU_TPMT_TK_HASHCHECK_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData,
                                               validation);
}

TSS2_RC
Tss2_Sys_Hash(TSS2_SYS_CONTEXT             *sysContext,
