...
TSS2_RC
Tss2_Sys_ContextLoad_Prepare(TSS2_SYS_CONTEXT *sysContext, const TPMS_CONTEXT *context) {
    ...
    rval = Tss2_MU_TPMS_CONTEXT_Marshal(context, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_ContextLoad_Complete(TSS2_SYS_CONTEXT *sysContext, TPMI_DH_CONTEXT *loadedHandle) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
    TSS2_RC                rval;
    ...
    rval = Tss2_MU_UINT32_Unmarshal(ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData, loadedHandle);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_ContextLoad(TSS2_SYS_CONTEXT   *sysContext,
