...
TSS2_RC
Tss2_Sys_PolicyOR_Prepare(TSS2_SYS_CONTEXT  *sysContext,
    ...
    rval = Tss2_MU_TPML_DIGEST_Marshal(pHashList, ctx->cmdBuffer, ctx->maxCmdSize, &ctx->nextData);
    if (rval)
        return rval;
...
TSS2_RC
Tss2_Sys_PolicyOR_Complete(TSS2_SYS_CONTEXT *sysContext) {
    TSS2_SYS_CONTEXT_BLOB *ctx = syscontext_cast(sysContext);
...
TSS2_RC
Tss2_Sys_PolicyOR(TSS2_SYS_CONTEXT             *sysContext,
                  TPMI_SH_POLICY                policySession,
