...
TSS2_RC
handle_from_name(const char *file, void **handle) {
    size_t      size = 0;
...
TSS2_RC
tcti_from_file(const char *file, const char *conf, TSS2_TCTI_CONTEXT **tcti, void **dlhandle) {
    TSS2_RC             r;
    void               *handle;
...
TSS2_RC
get_info_default(const TSS2_TCTI_INFO **info, void **dlhandle) {
    void                 *handle = NULL;
...
tctildr_get_default(TSS2_TCTI_CONTEXT **tcticontext, void **dlhandle) {
    ...
    *tcticontext = NULL;
    ...
    return tcti_from_file(ESYS_TCTI_DEFAULT_MODULE, config, tcticontext, dlhandle);
...
#pragma GCC diagnostic pop
        ...
        r = tcti_from_file(tctis[i].file, tctis[i].conf, tcticontext, dlhandle);
        if (r == TSS2_RC_SUCCESS)
            return TSS2_RC_SUCCESS;
...
TSS2_RC
info_from_name(const char *name, const TSS2_TCTI_INFO **info, void **data) {
    TSS2_RC rc;
