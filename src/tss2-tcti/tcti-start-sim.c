...
#include "util/log.h"         // for LOG_ERROR, LOG_TRACE, LOG_DEBUG, LOG_WA...
...
static int simulator_get_num_sockets(pid_t pid) {
    ...
    if (dir == NULL) {
        LOG_ERROR("opendir(%s) failed: %s", fd_dir_path, strerror(errno));
        return -1;
    }
    ...
    while (1) {
        ...
        if (len == -1) {
            LOG_ERROR("readlink failed for %s: %s", fd_link_path, strerror(errno));
            return -1;
        }
...
static int simulator_start_and_wait_for_ports(pid_t *simulator_pid, const tcti_sim_variant *variant, uint16_t port) {
    ...
    *simulator_pid = fork();
    ...
    if (*simulator_pid == 0) {
        ...
        r = simulator_execute_blocking(parent_pid, workdir, argv);
    ...
    while (1) {
        ...
        if (num_sockets >= variant->num_ports) {
            break;
        }
...
static int get_random_port() {
    ...
    if (!is_seeded) {
        /* seeding with seconds is not enough */
        struct timeval time;
...
tcti_start_sim_conf_parse_alloc(const char *conf, const tcti_sim_variant **variant, uint16_t *port, char **new_conf)
{
    ...
    *new_conf = NULL;
     ...
     *
     * Rules:
     *  - A valid config string can contain 0..n colons, thay split the string
     *    into elements
