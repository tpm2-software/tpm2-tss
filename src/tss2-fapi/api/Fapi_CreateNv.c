 ...
 */
TSS2_RC
Fapi_CreateNv(FAPI_CONTEXT *context,
              char const   *path,
              char const   *type,
              size_t        size,
    ...
    r = Fapi_CreateNv_Async(context, path, type, size, policyPath, authValue);
 ...
 */
TSS2_RC
Fapi_CreateNv_Async(FAPI_CONTEXT *context,
                    char const   *path,
                    char const   *type,
                    size_t        size,
                    char const   *policyPath,
    ...
    r = ifapi_set_nv_flags(type ? type : "", &nvCmd->public_templ, policyPath);
...
Fapi_CreateNv_Finish(FAPI_CONTEXT *context) {
    ...
    /* Mix the provided flags provided via the type with with template
       of the current active crypto profile. */
        r = ifapi_merge_profile_into_nv_template(context, &nvCmd->public_templ);
    ...
    statecase(context->state, NV_CREATE_READ_HIERARCHY)
        r = ifapi_keystore_load_finish(&context->keystore, &context->io, &nvCmd->auth_object);
        return_try_again(r);
        ...
        nvCmd->auth_object.public.handle = miscNv->hierarchy;

        /* Check if a policy is set for the NV index to be created. */
        if (miscNv->policyInstance && strcmp(miscNv->policyInstance, "") != 0)
    ...
    statecase(context->state, NV_CREATE_CALCULATE_POLICY)
        if (!nvCmd->skip_policy_computation) {
            ...
            r = ifapi_calculate_tree(context, miscNv->policyInstance, policy,
                                     miscNv->public.nvPublic.nameAlg, &context->policy.digest_idx,
            ...
            *nvCmdPolicy = calloc(1, sizeof(TPMS_POLICY));
            goto_if_null(*nvCmdPolicy, "Out of memory,", TSS2_FAPI_RC_MEMORY, error_cleanup);
            **(nvCmdPolicy) = *policy;

            authPolicy->size = context->policy.hash_size;
            memcpy(&authPolicy->buffer[0],
                   &policy->policyDigests.digests[context->policy.digest_idx].digest,
                   context->policy.hash_size);
            LOGBLOB_TRACE(&authPolicy->buffer[0], context->policy.hash_size, "Create Key Policy");
        }
        fallthrough;
    ...
    statecase(context->state, NV_CREATE_GET_INDEX)
        if (nvCmd->public_templ.public.nvIndex) {
            ...
            r = ifapi_get_nv_start_index(nvCmd->nvPath, &publicInfo->nvPublic.nvIndex);
    ...
    /* Get nv index if not already defined. */
        r = ifapi_get_free_handle_finish(context, &publicInfo->nvPublic.nvIndex, nvCmd->maxNvIndex);
        return_try_again(r);
        goto_if_error_reset_state(r, "FAPI get handle index.", error_cleanup);
    ...
    /* Authorize with the storage hierarchy "owner" for NV creation. */
        ...
        /* Create the NV Index. */
        r = Esys_NV_DefineSpace_Async(context->esys, hierarchy->public.handle, auth_session,
                                      ENC_SESSION_IF_POLICY(auth_session), ESYS_TR_NONE, auth,
                                      publicInfo);
    ...
    /* Store whether the NV index requires a password. */
        ...
        r = ifapi_keystore_object_does_not_exist(&context->keystore, nvCmd->nvPath,
                                                 &nvCmd->nv_object);
        goto_if_error_reset_state(r, "Could not write: %sh", error_cleanup, nvCmd->nvPath);

        /* Start writing the NV object to the key store */
        r = ifapi_keystore_store_async(&context->keystore, &context->io, nvCmd->nvPath,
                                       &nvCmd->nv_object);
    ...
    statecase(context->state, NV_CREATE_CHECK_EXISTING)
        r = ifapi_check_existing_nv(context, publicInfo->nvPublic.nvIndex, &nv_exists,
                                    &nvCmd->nv_object.public.handle, &existing_nv_public);
        return_try_again(r);
            ...
            if (existing_nv_public->nvPublic.authPolicy.size) {
                if (existing_nv_public->nvPublic.attributes & TPMA_NV_POLICYWRITE
                    ...
                    if (existing_nv_public->nvPublic.authPolicy.size
                        ...
                        goto_error(r, TSS2_FAPI_RC_BAD_VALUE, "The two policies do not match.",
                                   error_cleanup);
                    }
                    ...
                    goto_error(r, TSS2_FAPI_RC_BAD_VALUE,
                               "Object with policy can't be used in FAPI.", error_cleanup);
                }
            } else {
                context->state = NV_CREATE_SERIALIZE;
            }
