    ...
    if (context) {
        ...
        if (context->esys) {
            ...
            if (pctx->ek_tpm_handle && pctx->ek_esys_handle) {
                Esys_EvictControl(context->esys, ESYS_TR_RH_OWNER, pctx->ek_esys_handle,
                                  ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE, pctx->ek_tpm_handle,
                                  &dmy_handle);
            }
            if (pctx->srk_tpm_handle && pctx->srk_esys_handle) {
                Esys_EvictControl(context->esys, ESYS_TR_RH_OWNER, pctx->srk_esys_handle,
...
Fapi_Provision_Async(FAPI_CONTEXT *context,
    ...
    if (ifapi_io_path_exists(profile_dir)) {
        goto_error(r, TSS2_FAPI_RC_ALREADY_PROVISIONED, "Profile %s was already provisioned.", end,
                   context->keystore.defaultprofile);
    }
    ...
    r = ifapi_keystore_list_all(&context->keystore, "/", &command->pathlist, &command->numPaths);
...
Fapi_Provision_Finish(FAPI_CONTEXT *context) {
    ...
    statecase(context->state, PROVISION_GET_HIERARCHIES);
        command->hierarchies = calloc(1, command->numHierarchyObjects * sizeof(IFAPI_OBJECT));
        goto_if_null(command->hierarchies, "Out of memory", TSS2_FAPI_RC_MEMORY, error_cleanup);
        command->path_idx = 0;
        fallthrough;
    ...
    statecase(context->state, PROVISION_READ_HIERARCHIES);
        ...
        goto_if_error2(r, "Could not open %s", error_cleanup, command->pathlist[command->path_idx]);
        fallthrough;

    statecase(context->state, PROVISION_READ_HIERARCHY);
        path = command->pathlist[command->path_idx];
        if (path == NULL) {
            goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "Wrong path.", error_cleanup);
        }
        ...
        if (strcmp(path, "/HS") == 0) {
            ...
            goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "Invalid hierarchy list.", error_cleanup);
        }
    ...
    statecase(context->state, PROVISION_INIT);
        ...
        strdup_check(hierarchy_hs->misc.hierarchy.description, "Owner Hierarchy", r, error_cleanup);
        ifapi_init_hierarchy_object(hierarchy_he, ESYS_TR_RH_ENDORSEMENT);
        strdup_check(hierarchy_he->misc.hierarchy.description, "Endorsement Hierarchy", r,
                     error_cleanup);
        ifapi_init_hierarchy_object(hierarchy_lockout, ESYS_TR_RH_LOCKOUT);
        strdup_check(hierarchy_lockout->misc.hierarchy.description, "Lockout Hierarchy", r,
                     error_cleanup);
        ifapi_init_hierarchy_object(hierarchy_hn, ESYS_TR_RH_NULL);
        strdup_check(hierarchy_hn->misc.hierarchy.description, "Null Hierarchy", r, error_cleanup);
        fallthrough;
    ...
    statecase(context->state, PROVISION_PREPARE_GET_CAP_AUTH_STATE);
        ...
        /* Generate template for checking whether persistent SRK handle
           already exists. */
        r = ifapi_set_key_flags(defaultProfile->srk_template, false, &command->public_templ);
        ...
        r = Esys_GetCapability_Async(context->esys, ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                                     TPM2_CAP_TPM_PROPERTIES, TPM2_PT_PERMANENT, 1);
    ...
    statecase(context->state, PROVISION_WAIT_FOR_GET_CAP_AUTH_STATE);
        ...
        if ((*capabilityData)->data.tpmProperties.tpmProperty[0].property != TPM2_PT_PERMANENT) {
            SAFE_FREE(*capabilityData);
            goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "TPM2_PT_PERMANENT cannot be determined.",
                       error_cleanup);
        }
        ...
        if (command->public_templ.persistent_handle) {
            r = Esys_GetCapability_Async(context->esys, ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                                         TPM2_CAP_HANDLES, command->public_templ.persistent_handle,
                                         1);
            goto_if_error(r, "Esys_GetCapability_Async", error_cleanup);
        }
        fallthrough;
    ...
    statecase(context->state, PROVISION_WAIT_FOR_GET_CAP1);
        ...
        r = ifapi_check_profile_pcr_selection(&defaultProfile->pcr_selection, &pcr_capability);
    ...
    statecase(context->state, PROVISION_READ_EK_TEMPLATE);
        if (command->template_nv_index) {
            ...
            if (ek_template_size) {
                r = Tss2_MU_TPMT_PUBLIC_Unmarshal(ek_template, ek_template_size, NULL,
                                                  &command->public_templ.public.publicArea);
    ...
    statecase(context->state, PROVISION_INIT_GET_CAP2);
        ...
        if (hash_alg) {
            ...
            if (!(hash_size = ifapi_hash_get_digest_size(hash_alg))) {
                goto_error(r, TSS2_ESYS_RC_NOT_IMPLEMENTED,
                           "Unsupported hash algorithm (%" PRIu16 ")", error_cleanup, hash_alg);
            }
        ...
        if (context->config.ek_cert_file) {
            ...
            if (curl_rc != 0) {
                goto_error_reset_state(r, TSS2_FAPI_RC_NO_CERT, "Get certificate.", error_cleanup);
            }

            /* Get the public key from the certificate. */
            r = ifapi_get_public_from_pem_cert(command->pem_cert, &public_key);
            goto_if_error_reset_state(r, "Get public key from pem certificate", error_cleanup);
            ...
            goto_error(r, TSS2_FAPI_RC_BAD_KEY, "Public key of EK does not match certificate.",
                       error_cleanup);
        }

        /* Prepare the search for certificates store in the NV ram of the TPM. */
        r = Esys_GetCapability_Async(context->esys, ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE,
                                     TPM2_CAP_HANDLES, MIN_EK_CERT_HANDLE, TPM2_MAX_CAP_HANDLES);
        goto_if_error(r, "Esys_GetCapability_Async", error_cleanup);
    ...
    statecase(context->state, PROVISION_GET_CERT_NV);
        ...
        r = Esys_TR_FromTPMPublic_Async(context->esys, command->cert_nv_idx, ESYS_TR_NONE,
    ...
    statecase(context->state, PROVISION_GET_CERT_NV_FINISH);
        r = Esys_TR_FromTPMPublic_Finish(context->esys, &command->esys_nv_cert_handle);
        return_try_again(r);
        goto_if_error_reset_state(r, "TR_FromTPMPublic_Finish", error_cleanup);

        /* Read public to get size of certificate */
        r = Esys_NV_ReadPublic_Async(context->esys, command->esys_nv_cert_handle, ESYS_TR_NONE,
                                     ESYS_TR_NONE, ESYS_TR_NONE);
    ...
    statecase(context->state, PROVISION_READ_CERT);
        ...
        *hierarchy_hs = nvCmd->auth_object;
        ...
        r = ifapi_cert_to_pem(certData, certSize, &command->pem_cert, &command->cert_key_type,
                              &public_key);
        SAFE_FREE(certData);
        ...
        goto_error(r, TSS2_FAPI_RC_NO_CERT, "No EK certificate found.", error_cleanup);

    statecase(context->state, PROVISION_READ_CERT_CHAIN);
        /* Retrieve the certificate chains from the TPM's NV space . */
        r = ifapi_get_certificates(context, EK_CERT_CHAIN_MIN, EK_CERT_CHAIN_MAX, &command->certs,
...
#pragma message("*** Allow self generated certifcate ***")
        ...
        return_try_again(r);
        goto_if_error2(r, "Reading certificate %s", error_cleanup, getenv("FAPI_TEST_INT_CERT"));
    ...
    statecase(context->state, PROVISION_EK_CHECK_CERT);
        ...
        r = ifapi_verify_cert_chain(command->pem_cert, command->certs, command->cert_list_size,
    ...
    statecase(context->state, PROVISION_EK_WRITE_PREPARE);
        ...
        r = ifapi_keystore_store_async(&context->keystore, &context->io, "HE/EK", pkeyObject);
    ...
    statecase(context->state, PROVISION_AUTHORIZE_LOCKOUT);
        ...
        r = Esys_DictionaryAttackParameters_Async(
            context->esys, ESYS_TR_RH_LOCKOUT, auth_session, ESYS_TR_NONE, ESYS_TR_NONE,
            defaultProfile->newMaxTries, defaultProfile->newRecoveryTime,
            defaultProfile->lockoutRecovery);
        goto_if_error(r, "Error Esys_DictionaryAttackParameters", error_cleanup);
    ...
    statecase(context->state, PROVISION_AUTH_SRK_NO_AUTH_SENT);
        ...
        if (command->public_templ.persistent_handle & command->srk_exists) {
            ...
            r = Esys_TR_FromTPMPublic_Async(context->esys, command->public_templ.persistent_handle,
    ...
    statecase(context->state, PROVISION_SRK_GET_PERSISTENT_NAME);
        ...
        r = Esys_TR_GetName(context->esys, srk_persistent_handle, &srk_name_persistent);
        goto_if_error(r, "Get srk name", error_cleanup);

        /* Compare the name of the generated key with the name of the
           persistent key. */
        if (srk_name->size != srk_name_persistent->size
            || memcmp(&srk_name->name[0], &srk_name_persistent->name[0], srk_name->size) != 0) {
            /* The persistent key cannot be used. */
            goto_error(r, TSS2_FAPI_RC_BAD_VALUE, "SRK persistent handle already defined",
                       error_cleanup);
        }
    ...
    statecase(context->state, PROVISION_SRK_WRITE_PREPARE);
        ...
        r = ifapi_keystore_store_async(&context->keystore, &context->io, "HS/SRK", pkeyObject);
    ...
    statecase(context->state, PROVISION_WAIT_FOR_EK_SESSION);
        ...
        if (strlen(command->authValueLockout) > sizeof(TPMU_HA)) {
            goto_error(r, TSS2_FAPI_RC_BAD_VALUE, "Password too long.", error_cleanup);
        }
        /* Copy auth value into context. */
        memcpy(&command->hierarchy_auth.buffer[0], command->authValueLockout,
               strlen(command->authValueLockout));
        ...
        if (defaultProfile->lockout_policy) {
            command->hierarchy_policy = ifapi_copy_policy(defaultProfile->lockout_policy);
            goto_if_null2(command->hierarchy_policy, "Out of memory.", r, TSS2_FAPI_RC_MEMORY,
                          error_cleanup);
        } else {
    ...
    statecase(context->state, PROVISION_WAIT_FOR_EK_PERSISTENT);
        ...
        if (number_rc(r) == TPM2_RC_BAD_AUTH && hierarchy_hs->misc.hierarchy.with_auth == TPM2_NO) {
            hierarchy_hs->misc.hierarchy.with_auth = TPM2_YES;
    ...
    statecase(context->state, PROVISION_CHANGE_LOCKOUT_AUTH);
        ...
        r = ifapi_change_auth_hierarchy(context, ESYS_TR_RH_LOCKOUT, &command->hierarchy_lockout,
                                        &command->hierarchy_auth);
        return_try_again(r);
    ...
    statecase(context->state, PROVISION_LOCKOUT_CHANGE_POLICY);
        ...
        r = ifapi_change_policy_hierarchy(context, ESYS_TR_RH_LOCKOUT, hierarchy_lockout,
        ...
        r = ifapi_keystore_store_async(&context->keystore, &context->io, "/LOCKOUT",
                                       &command->hierarchy_lockout);
        goto_if_error_reset_state(r, "Could not open: %s", error_cleanup, "/LOCKOUT");
    ...
    statecase(context->state, PROVISION_CHANGE_EH_AUTH);
        ...
        r = ifapi_change_auth_hierarchy(context, ESYS_TR_RH_ENDORSEMENT, &command->hierarchy_he,
                                        &command->hierarchy_auth);
        return_try_again(r);
        goto_if_error(r, "Change auth hierarchy.", error_cleanup);
        fallthrough;
    ...
    statecase(context->state, PROVISION_EH_CHANGE_POLICY);
        ...
        r = ifapi_change_policy_hierarchy(context, ESYS_TR_RH_ENDORSEMENT, hierarchy_he,
                                          command->hierarchy_policy);
        return_try_again(r);
        goto_if_error(r, "Change policy EH", error_cleanup);

        /* Check whether object already exists in key store.*/
        r = ifapi_keystore_object_does_not_exist(&context->keystore, "/HE", &command->hierarchy_he);
        ...
        r = ifapi_keystore_store_async(&context->keystore, &context->io, "/HE", hierarchy_he);
    ...
    statecase(context->state, PROVISION_CHANGE_SH_AUTH);
        /* If an owner hierarchy auth value is provided by profile,
           the auth value will be changed asynchronous. */
        context->hierarchy_policy_state = HIERARCHY_CHANGE_POLICY_INIT;
        r = ifapi_change_auth_hierarchy(context, ESYS_TR_RH_OWNER, &command->hierarchy_hs,
                                        &command->hierarchy_auth);
        return_try_again(r);
        goto_if_error(r, "Change auth hierarchy.", error_cleanup);
    ...
    statecase(context->state, PROVISION_SH_CHANGE_POLICY);
        /* If an owner hierarchy policy is provided by profile,
           the policy will be assigned asynchronous. */
        r = ifapi_change_policy_hierarchy(context, ESYS_TR_RH_OWNER, hierarchy_hs,
                                          command->hierarchy_policy);
        return_try_again(r);
        goto_if_error(r, "Change policy SH", error_cleanup);

        /* Check whether object already exists in key store.*/
        r = ifapi_keystore_object_does_not_exist(&context->keystore, "/HS", hierarchy_hs);
    ...
    statecase(context->state, PROVISION_WRITE_HIERARCHIES);
        ...
        if (command->hierarchies[command->path_idx].public.handle
            == command->hierarchy->public.handle) {
    ...
    statecase(context->state, PROVISION_GET_VENDOR);
        ...
        if ((*capabilityData)->data.tpmProperties.tpmProperty[0].value == VENDOR_INTC
            ...
            size_t   cert_size;
            TPM2B_PUBLIC public;
            r = ifapi_get_web_ek_certificate(
                context, &pkey->public, (*capabilityData)->data.tpmProperties.tpmProperty[0].value,
                &cert_buffer, &cert_size);
            goto_if_error_reset_state(r, "Get certificates", error_cleanup);

            r = ifapi_cert_to_pem(cert_buffer, cert_size, &command->pem_cert, NULL, &public);
            SAFE_FREE(cert_buffer);
            goto_if_error_reset_state(r, "Convert certificate buffer to PEM.", error_cleanup);

            r = ifapi_curl_verify_ek_cert(NULL, NULL, command->pem_cert);
            goto_if_error_reset_state(r, "Verify EK certificate.", error_cleanup);

        } else {
            /* No certificate was stored in the TPM and ek_cert_less was not set.*/
            goto_error(
                r, TSS2_FAPI_RC_NO_CERT,
...
error_cleanup:
    ...
    SAFE_FREE(command->root_crt);
    SAFE_FREE(*capabilityData);
    secure_char_zero(command->authValueLockout);
    SAFE_FREE(command->authValueLockout);
    secure_char_zero(command->authValueLockout);
    SAFE_FREE(command->authValueEh);
    secure_char_zero(command->authValueLockout);
