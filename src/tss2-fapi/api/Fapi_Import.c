...
Fapi_Import_Async(FAPI_CONTEXT *context, char const *path, char const *importData) {
    ...
    if (strncmp(importData, IFAPI_PEM_PUBLIC_STRING, sizeof(IFAPI_PEM_PUBLIC_STRING) - 1) == 0) {
        ...
        TPM2_ALG_ID rsaOrEcc = ifapi_get_signature_algorithm_from_pem(extPubKey->pem_ext_public);
        r = ifapi_initialize_sign_public(rsaOrEcc, &extPubKey->public);
        goto_if_error(r, "Could not initialize key template", cleanup_error);

        r = ifapi_get_tpm2b_public_from_pem(extPubKey->pem_ext_public, &extPubKey->public);
        ...
        if (strncmp(&path[pos], IFAPI_PUB_KEY_DIR, strlen(IFAPI_PUB_KEY_DIR)) != 0) {
            ...
            r = ifapi_asprintf(&command->out_path, "%s%s%s", IFAPI_PUB_KEY_DIR, IFAPI_FILE_DELIM,
                               &path[pos]);
    ...
    } else if (strncmp(importData, IFAPI_PEM_PRIVATE_KEY, sizeof(IFAPI_PEM_PRIVATE_KEY) - 1) == 0
               || strncmp(importData, IFAPI_PEM_ECC_PRIVATE_KEY,
                          sizeof(IFAPI_PEM_ECC_PRIVATE_KEY) - 1)
                      == 0
               || strncmp(importData, IFAPI_PEM_RSA_PRIVATE_KEY,
                          sizeof(IFAPI_PEM_RSA_PRIVATE_KEY) - 1)
                      == 0) {
       ...
       to block until a result is available. */
        ...
        if (ifapi_get_sub_object(jso, IFAPI_JSON_TAG_POLICY, &jso2)
            ...
            r = ifapi_policystore_check_overwrite(&context->pstore, command->out_path);
            goto_if_error_reset_state(r, "Check overwrite %s", cleanup_error, command->out_path);

            r = ifapi_policy_store_store_async(&context->pstore, &context->io, command->out_path,
                                               &policy);
            goto_if_error_reset_state(r, "Could not open: %s", cleanup_error, command->out_path);
            ...
            case IFAPI_EXT_PUB_KEY_OBJ:
                /* Check whether an existing object would be overwritten */
                r = ifapi_keystore_check_overwrite(&context->keystore, command->out_path);
                ...
                r = ifapi_keystore_store_async(&context->keystore, &context->io, command->out_path,
                                               object);
            ...
            case IFAPI_DUPLICATE_OBJ:
                r = ifapi_get_name(&keyTree->public_parent.publicArea, &command->parent_name);
            ...
            default:
                goto_error(r, TSS2_FAPI_RC_GENERAL_FAILURE, "Invalid object type", cleanup_error);
                break;
            }
            command->parent_path = NULL;
        } else {
            ...
            r = ifapi_get_name(&object->misc.key.public.publicArea, &object->misc.key.name);
            goto_if_error2(r, "Get parent name", cleanup_error);
...
Fapi_Import_Finish(FAPI_CONTEXT *context) {
    ...
    statecase(context->state, IMPORT_WAIT_FOR_SESSION);
        ...
        r = ifapi_profiles_get(&context->profiles, profile_name, &context->cmd.ImportKey.profile);
    ...
    statecase(context->state, IMPORT_WAIT_FOR_PARENT);
        ...
        if (command->ossl_priv) {
            ...
            r = ifapi_openssl_load_private(command->ossl_priv, NULL, NULL,
                                           &context->cmd.ImportKey.public_templ.public,
                                           &keyTree->public, &command->sensitive);
            ...
            r = Tss2_MU_UINT16_Marshal(marshalled_sensitive_size, &keyTree->duplicate.buffer[0],
                                       sizeof(uint16_t), &marshalled_length);
    ...
    statecase(context->state, IMPORT_WAIT_FOR_KEY);
        ...
        r = ifapi_keystore_check_overwrite(&context->keystore, command->out_path);
        goto_if_error_reset_state(r, "Check overwrite %s", error_cleanup, command->out_path);

        /* Start writing the object to the key store */
        r = ifapi_keystore_store_async(&context->keystore, &context->io, command->out_path, object);
        goto_if_error_reset_state(r, "Could not open: %sh", error_cleanup, command->out_path);
        ifapi_cleanup_ifapi_object(object);
        fallthrough;
    ...
    statecase(context->state, IMPORT_KEY_SEARCH);
        r = ifapi_keystore_search_obj(&context->keystore, &context->io, &command->parent_name,
                                      &command->parent_path);
        return_try_again(r);
    ...
    statecase(context->state, IMPORT_KEY_LOAD_PARENT);
        r = ifapi_load_key(context, command->parent_path, &command->parent_object);
        return_try_again(r);
    ...
    statecase(context->state, IMPORT_KEY_AUTHORIZE_PARENT);
        ...
        r = Esys_Import_Async(context->esys, command->parent_object->public.handle, session,
                              ENC_SESSION_IF_POLICY(session), ESYS_TR_NONE, NULL, &keyTree->public,
                              &keyTree->duplicate, &keyTree->encrypted_seed, &symmetric);
    ...
    statecase(context->state, IMPORT_KEY_WAIT_FOR_FLUSH);
        ...
        r = ifapi_get_profile_sig_scheme(
            context->cmd.ImportKey.profile ? context->cmd.ImportKey.profile
                                           : &context->profiles.default_profile,
            &keyTree->public.publicArea, &newObject->misc.key.signing_scheme);
    ...
    statecase(context->state, IMPORT_KEY_WRITE_OBJECT_PREPARE);
        ...
        r = ifapi_keystore_store_async(&context->keystore, &context->io, command->out_path,
                                       newObject);
        goto_if_error_reset_state(r, "Could not open: %s", error_cleanup, command->out_path);
