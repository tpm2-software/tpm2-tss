    ...
    TSS2_RC Tss2_MU_##type##_Marshal(type const *src, uint8_t buffer[], size_t buffer_size,        \
        ...
        if (buffer == NULL && offset == NULL) {                                                    \
            ...
            *offset += sizeof(src->size) + src->size;                                              \
            ...
            LOG_WARNING("size: %u for buffer of " #type " is larger than max length"               \
                        " of buffer: %zu",                                                         \
                        src->size, (sizeof(type) - sizeof(src->size)));                            \
...
#define TPM2B_UNMARSHAL(type, buf_name)                                                            \
    TSS2_RC Tss2_MU_##type##_Unmarshal(uint8_t const buffer[], size_t buffer_size, size_t *offset, \
        ...
        if (buffer == NULL || (dest == NULL && offset == NULL)) {                                  \
            ...
            return TSS2_MU_RC_BAD_REFERENCE;                                                       \
        } else if (buffer_size < local_offset || sizeof(size) > buffer_size - local_offset) {      \
            LOG_DEBUG("buffer_size: %zu with offset: %zu are insufficient for object "             \
                      "of size %zu",                                                               \
                      buffer_size, local_offset, sizeof(size));                                    \
            return TSS2_MU_RC_INSUFFICIENT_BUFFER;                                                 \
        }                                                                                          \
                                                                                                   \
        ...
        LOG_DEBUG("Unmarshaling " #type " from 0x%" PRIxPTR " to buffer 0x%" PRIxPTR               \
                  " at index 0x%zx, buffer size %zu, object size %u",                              \
                  (uintptr_t)buffer, (uintptr_t)dest, local_offset, buffer_size, size);            \
                                                                                                   \
        if (size > buffer_size - local_offset) {                                                   \
            LOG_DEBUG("buffer_size: %zu with offset: %zu are insufficient for object "             \
                      "of size %zu",                                                               \
                      buffer_size, local_offset, (size_t)size);                                    \
            return TSS2_MU_RC_INSUFFICIENT_BUFFER;                                                 \
        }                                                                                          \
...
#define TPM2B_MARSHAL_SUBTYPE(type, subtype, member)                                               \
    TSS2_RC Tss2_MU_##type##_Marshal(type const *src, uint8_t buffer[], size_t buffer_size,        \
        ...
        if (buffer == NULL && offset == NULL) {                                                    \
            LOG_WARNING("buffer and offset parameter are NULL");                                   \
            return TSS2_MU_RC_BAD_REFERENCE;                                                       \
        } else if (buffer_size < local_offset || buffer_size - local_offset < sizeof(src->size)) { \
            LOG_DEBUG("buffer_size: %zu with offset: %zu are insufficient for object "             \
                      "of size %zu",                                                               \
