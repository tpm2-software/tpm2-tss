...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_HMAC, Esys_...
...
Esys_HMAC(ESYS_CONTEXT           *esysContext,
    ...
    r = Esys_HMAC_Async(esysContext, handle, shandle1, shandle2, shandle3, buffer, hashAlg);
...
Esys_HMAC_Async(ESYS_CONTEXT           *esysContext,
    ...
    r = Tss2_Sys_HMAC_Prepare(esysContext->sys,
                              (handleNode == NULL) ? TPM2_RH_NULL : handleNode->rsrc.handle, buffer,
                              hashAlg);
...
Esys_HMAC_Finish(ESYS_CONTEXT *esysContext, TPM2B_DIGEST **outHMAC) {
    TSS2_RC r;
    LOG_TRACE("context=%p, outHMAC=%p", esysContext, outHMAC);
     ...
     */
    r = Tss2_Sys_HMAC_Complete(esysContext->sys, (outHMAC != NULL) ? *outHMAC : NULL);
