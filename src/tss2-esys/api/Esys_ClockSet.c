...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_ClockSet
...
Esys_ClockSet(ESYS_CONTEXT *esysContext,
    ...
    r = Esys_ClockSet_Async(esysContext, auth, shandle1, shandle2, shandle3, newTime);
...
Esys_ClockSet_Async(ESYS_CONTEXT *esysContext,
                    ...
                    UINT64        newTime) {
    TSS2_RC r;
    LOG_TRACE("context=%p, auth=%" PRIx32 ", newTime=%" PRIx64 "", esysContext, auth, newTime);
    ...
    r = Tss2_Sys_ClockSet_Prepare(
        esysContext->sys, (authNode == NULL) ? TPM2_RH_NULL : authNode->rsrc.handle, newTime);
