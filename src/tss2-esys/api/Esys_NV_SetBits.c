...
#include <inttypes.h> // for PRIx32, PRIx64, int32_t
...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_NV_SetBits
#include "tss2_sys.h"        // for Tss2_Sys_ExecuteAsync, TSS2L_SYS_AUTH_C...
#include "tss2_tpm2_types.h" // for TPM2_RH_NULL, TPM2B_NV_PUBLIC, TPM2_RC_...
...
Esys_NV_SetBits(ESYS_CONTEXT *esysContext,
    ...
    r = Esys_NV_SetBits_Async(esysContext, authHandle, nvIndex, shandle1, shandle2, shandle3, bits);
...
Esys_NV_SetBits_Async(ESYS_CONTEXT *esysContext,
    ...
    r = Tss2_Sys_NV_SetBits_Prepare(
        esysContext->sys, (authHandleNode == NULL) ? TPM2_RH_NULL : authHandleNode->rsrc.handle,
        (nvIndexNode == NULL) ? TPM2_RH_NULL : nvIndexNode->rsrc.handle, bits);
