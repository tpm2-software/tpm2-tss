...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_PolicyLocality
#include "tss2_sys.h"        // for Tss2_Sys_ExecuteAsync, TSS2L_SYS_AUTH_C...
#include "tss2_tpm2_types.h" // for TPMA_LOCALITY, TPM2_RC_RETRY, TPM2_RC_T...
...
Esys_PolicyLocality(ESYS_CONTEXT *esysContext,
    ...
    r = Esys_PolicyLocality_Async(esysContext, policySession, shandle1, shandle2, shandle3,
                                  locality);
...
Esys_PolicyLocality_Async(ESYS_CONTEXT *esysContext,
                          ...
                          TPMA_LOCALITY locality) {
    TSS2_RC r;
    LOG_TRACE("context=%p, policySession=%" PRIx32 ", locality=%02" PRIx8 "", esysContext,
              policySession, locality);
    ...
    r = Tss2_Sys_PolicyLocality_Prepare(
        esysContext->sys,
        (policySessionNode == NULL) ? TPM2_RH_NULL : policySessionNode->rsrc.handle, locality);
