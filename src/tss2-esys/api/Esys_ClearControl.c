...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_ClearControl
...
Esys_ClearControl(ESYS_CONTEXT *esysContext,
    ...
    r = Esys_ClearControl_Async(esysContext, auth, shandle1, shandle2, shandle3, disable);
...
Esys_ClearControl_Async(ESYS_CONTEXT *esysContext,
                        ...
                        TPMI_YES_NO   disable) {
    TSS2_RC r;
    LOG_TRACE("context=%p, auth=%" PRIx32 ", disable=%02" PRIx8 "", esysContext, auth, disable);
    ...
    r = Tss2_Sys_ClearControl_Prepare(
        esysContext->sys, (authNode == NULL) ? TPM2_RH_NULL : authNode->rsrc.handle, disable);
