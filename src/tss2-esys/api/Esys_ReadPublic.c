...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_ReadPublic
#include "tss2_sys.h"        // for Tss2_Sys_ExecuteAsync, TSS2L_SYS_AUTH_C...
#include "tss2_tpm2_types.h" // for TPM2B_NAME, TPM2B_PUBLIC, TPM2_RC_RETRY
...
Esys_ReadPublic(ESYS_CONTEXT  *esysContext,
    ...
    r = Esys_ReadPublic_Async(esysContext, objectHandle, shandle1, shandle2, shandle3);
...
Esys_ReadPublic_Async(ESYS_CONTEXT *esysContext,
                      ...
                      ESYS_TR       shandle3) {
    TSS2_RC r;
    LOG_TRACE("context=%p, objectHandle=%" PRIx32 "", esysContext, objectHandle);
    ...
    r = Tss2_Sys_ReadPublic_Prepare(esysContext->sys, (objectHandleNode == NULL)
                                                          ? TPM2_RH_NULL
...
Esys_ReadPublic_Finish(ESYS_CONTEXT  *esysContext,
    ...
    LOG_TRACE("context=%p, outPublic=%p, name=%p,"
              "qualifiedName=%p",
              esysContext, outPublic, name, qualifiedName);
     ...
     */
    r = Tss2_Sys_ReadPublic_Complete(esysContext->sys, (outPublic != NULL) ? *outPublic : NULL,
                                     (name != NULL) ? *name : NULL,
