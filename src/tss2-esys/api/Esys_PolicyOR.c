...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_PolicyOR
#include "tss2_sys.h"        // for Tss2_Sys_ExecuteAsync, TSS2L_SYS_AUTH_C...
#include "tss2_tpm2_types.h" // for TPML_DIGEST, TPM2_RC_RETRY, TPM2_RC_TES...
...
Esys_PolicyOR(ESYS_CONTEXT      *esysContext,
    ...
    r = Esys_PolicyOR_Async(esysContext, policySession, shandle1, shandle2, shandle3, pHashList);
...
Esys_PolicyOR_Async(ESYS_CONTEXT      *esysContext,
                    ...
                    const TPML_DIGEST *pHashList) {
    TSS2_RC r;
    LOG_TRACE("context=%p, policySession=%" PRIx32 ", pHashList=%p", esysContext, policySession,
              pHashList);
    ...
    r = Tss2_Sys_PolicyOR_Prepare(
        esysContext->sys,
        (policySessionNode == NULL) ? TPM2_RH_NULL : policySessionNode->rsrc.handle, pHashList);
