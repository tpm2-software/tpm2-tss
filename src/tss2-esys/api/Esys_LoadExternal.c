...
#include "tss2_tpm2_types.h" // for TPM2B_PUBLIC, TPM2B_SENSITIVE, TPM2B_NAME
...
static void
store_input_parameters(ESYS_CONTEXT *esysContext, const TPM2B_PUBLIC *inPublic) {
    if (inPublic == NULL) {
        esysContext->in.LoadExternal.inPublic = NULL;
    } else {
        esysContext->in.LoadExternal.inPublicData = *inPublic;
        esysContext->in.LoadExternal.inPublic = &esysContext->in.LoadExternal.inPublicData;
    }
}
...
Esys_LoadExternal(ESYS_CONTEXT          *esysContext,
    ...
    r = Esys_LoadExternal_Async(esysContext, shandle1, shandle2, shandle3, inPrivate, inPublic,
                                hierarchy);
...
Esys_LoadExternal_Async(ESYS_CONTEXT          *esysContext,
    ...
    r = Tss2_Sys_LoadExternal_Prepare(esysContext->sys, inPrivate, inPublic, tpm_hierarchy);
...
Esys_LoadExternal_Finish(ESYS_CONTEXT *esysContext, ESYS_TR *objectHandle) {
    TSS2_RC r;
    LOG_TRACE("context=%p, objectHandle=%p", esysContext, objectHandle);
    ...
    *objectHandle = esysContext->esys_handle_cnt++;
     ...
     */
    r = Tss2_Sys_LoadExternal_Complete(esysContext->sys, &objectHandleNode->rsrc.handle, &name);
    goto_state_if_error(r, ESYS_STATE_INTERNALERROR, "Received error from SAPI unmarshaling",
                        error_cleanup);

    /* check name against inPublic */
    if (!iesys_compare_name(&esysContext->crypto_backend, esysContext->in.LoadExternal.inPublic,
                            &name)) {
