...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_GetCommandA...
...
Esys_GetCommandAuditDigest(ESYS_CONTEXT          *esysContext,
    ...
    r = Esys_GetCommandAuditDigest_Async(esysContext, privacyHandle, signHandle, shandle1, shandle2,
                                         shandle3, qualifyingData, inScheme);
     ...
     */
    do {
        r = Esys_GetCommandAuditDigest_Finish(esysContext, auditInfo, signature);
...
Esys_GetCommandAuditDigest_Async(ESYS_CONTEXT          *esysContext,
    ...
    return_state_if_error(r, ESYS_STATE_INIT, "privacyHandle unknown.");
    ...
    r = Tss2_Sys_GetCommandAuditDigest_Prepare(
        esysContext->sys,
        (privacyHandleNode == NULL) ? TPM2_RH_NULL : privacyHandleNode->rsrc.handle,
        (signHandleNode == NULL) ? TPM2_RH_NULL : signHandleNode->rsrc.handle, qualifyingData,
        inScheme);
    ...
    if (privacyHandleNode != NULL)
        iesys_compute_session_value(esysContext->session_tab[0], &privacyHandleNode->rsrc.name,
                                    &privacyHandleNode->auth);
    else
