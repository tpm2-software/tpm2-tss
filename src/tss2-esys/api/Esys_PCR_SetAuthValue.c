...
#include "tss2_tpm2_types.h" // for TPM2B_AUTH, TPM2B_DIGEST, TPM2_RC_RETRY
...
static void
store_input_parameters(ESYS_CONTEXT *esysContext, const ESYS_TR pcrHandle, const TPM2B_AUTH *auth) {
    if (auth == NULL)
        memset(&esysContext->in.PCR.authData, 0, sizeof(esysContext->in.PCR.authData));
    else
...
Esys_PCR_SetAuthValue(ESYS_CONTEXT       *esysContext,
    ...
    r = Esys_PCR_SetAuthValue_Async(esysContext, pcrHandle, shandle1, shandle2, shandle3, auth);
...
Esys_PCR_SetAuthValue_Async(ESYS_CONTEXT       *esysContext,
                            ...
                            const TPM2B_DIGEST *auth) {
    TSS2_RC r;
    LOG_TRACE("context=%p, pcrHandle=%" PRIx32 ", auth=%p", esysContext, pcrHandle, auth);
    ...
    r = Tss2_Sys_PCR_SetAuthValue_Prepare(
