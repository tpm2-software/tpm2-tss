...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_ReadClock
#include "tss2_sys.h"        // for Tss2_Sys_ExecuteAsync, TSS2L_SYS_AUTH_C...
#include "tss2_tpm2_types.h" // for TPMS_TIME_INFO, TPM2_RC_RETRY, TPM2_RC_...
...
Esys_ReadClock_Finish(ESYS_CONTEXT *esysContext, TPMS_TIME_INFO **currentTime) {
    TSS2_RC r;
    LOG_TRACE("context=%p, currentTime=%p", esysContext, currentTime);
     ...
     */
    r = Tss2_Sys_ReadClock_Complete(esysContext->sys, (currentTime != NULL) ? *currentTime : NULL);
