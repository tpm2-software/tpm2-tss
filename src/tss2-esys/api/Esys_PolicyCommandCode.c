...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_PolicyComma...
#include "tss2_sys.h"        // for Tss2_Sys_ExecuteAsync, TSS2L_SYS_AUTH_C...
#include "tss2_tpm2_types.h" // for TPM2_CC, TPM2_RC_RETRY, TPM2_RC_TESTING
...
Esys_PolicyCommandCode(ESYS_CONTEXT *esysContext,
    ...
    r = Esys_PolicyCommandCode_Async(esysContext, policySession, shandle1, shandle2, shandle3,
                                     code);
...
Esys_PolicyCommandCode_Async(ESYS_CONTEXT *esysContext,
                             ...
                             TPM2_CC       code) {
    TSS2_RC r;
    LOG_TRACE("context=%p, policySession=%" PRIx32 ", code=%" PRIx32 "", esysContext, policySession,
              code);
    ...
    r = Tss2_Sys_PolicyCommandCode_Prepare(
        esysContext->sys,
        (policySessionNode == NULL) ? TPM2_RH_NULL : policySessionNode->rsrc.handle, code);
