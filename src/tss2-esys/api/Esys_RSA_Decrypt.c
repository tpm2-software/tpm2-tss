...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_RSA_Decrypt
...
Esys_RSA_Decrypt(ESYS_CONTEXT               *esysContext,
    ...
    r = Esys_RSA_Decrypt_Async(esysContext, keyHandle, shandle1, shandle2, shandle3, cipherText,
                               inScheme, label);
...
Esys_RSA_Decrypt_Async(ESYS_CONTEXT               *esysContext,
    ...
    r = Tss2_Sys_RSA_Decrypt_Prepare(
...
Esys_RSA_Decrypt_Finish(ESYS_CONTEXT *esysContext, TPM2B_PUBLIC_KEY_RSA **message) {
    TSS2_RC r;
    LOG_TRACE("context=%p, message=%p", esysContext, message);
     ...
     */
    r = Tss2_Sys_RSA_Decrypt_Complete(esysContext->sys, (message != NULL) ? *message : NULL);
