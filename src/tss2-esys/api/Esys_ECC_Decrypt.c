...
Esys_ECC_Decrypt(ESYS_CONTEXT           *esysContext,
    ...
    r = Esys_ECC_Decrypt_Async(esysContext, keyHandle, shandle1, shandle2, shandle3, c1, c2, c3,
                               inScheme);
...
Esys_ECC_Decrypt_Async(ESYS_CONTEXT           *esysContext,
    ...
    r = Tss2_Sys_ECC_Decrypt_Prepare(
        esysContext->sys, (keyHandleNode == NULL) ? TPM2_RH_NULL : keyHandleNode->rsrc.handle, c1,
        c2, c3, inScheme);
...
Esys_ECC_Decrypt_Finish(ESYS_CONTEXT *esysContext, TPM2B_MAX_BUFFER **plainText) {
    TSS2_RC r;
    LOG_TRACE("context=%p, plainText=%p", esysContext, plainText);
     ...
     */
    r = Tss2_Sys_ECC_Decrypt_Complete(esysContext->sys, (plainText != NULL) ? *plainText : NULL);
