...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_MAC, Esys_M...
...
Esys_MAC_Finish(ESYS_CONTEXT *esysContext, TPM2B_DIGEST **outMAC) {
    TSS2_RC r;
    LOG_TRACE("context=%p, outMAC=%p", esysContext, outMAC);
     ...
     */
    r = Tss2_Sys_MAC_Complete(esysContext->sys, (outMAC != NULL) ? *outMAC : NULL);
