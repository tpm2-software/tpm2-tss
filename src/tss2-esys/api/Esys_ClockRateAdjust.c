...
#include <inttypes.h> // for PRIx32, PRIi8, int32_t
...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_ClockRateAd...
#include "tss2_sys.h"        // for Tss2_Sys_ExecuteAsync, TSS2L_SYS_AUTH_C...
#include "tss2_tpm2_types.h" // for TPM2_CLOCK_ADJUST, TPM2_RC_RETRY, TPM2_...
...
Esys_ClockRateAdjust(ESYS_CONTEXT     *esysContext,
    ...
    r = Esys_ClockRateAdjust_Async(esysContext, auth, shandle1, shandle2, shandle3, rateAdjust);
...
Esys_ClockRateAdjust_Async(ESYS_CONTEXT     *esysContext,
                           ...
                           TPM2_CLOCK_ADJUST rateAdjust) {
    TSS2_RC r;
    LOG_TRACE("context=%p, auth=%" PRIx32 ", rateAdjust=%" PRIi8 "", esysContext, auth, rateAdjust);
    ...
    r = Tss2_Sys_ClockRateAdjust_Prepare(
        esysContext->sys, (authNode == NULL) ? TPM2_RH_NULL : authNode->rsrc.handle, rateAdjust);
