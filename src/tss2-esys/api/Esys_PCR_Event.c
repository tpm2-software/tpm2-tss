...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_PCR_Event
#include "tss2_sys.h"        // for Tss2_Sys_ExecuteAsync, TSS2L_SYS_AUTH_C...
#include "tss2_tpm2_types.h" // for TPML_DIGEST_VALUES, TPM2B_EVENT, TPM2_R...
...
Esys_PCR_Event(ESYS_CONTEXT        *esysContext,
    ...
    r = Esys_PCR_Event_Async(esysContext, pcrHandle, shandle1, shandle2, shandle3, eventData);
...
Esys_PCR_Event_Async(ESYS_CONTEXT      *esysContext,
                     ...
                     const TPM2B_EVENT *eventData) {
    TSS2_RC r;
    LOG_TRACE("context=%p, pcrHandle=%" PRIx32 ", eventData=%p", esysContext, pcrHandle, eventData);
    ...
    return_state_if_error(r, ESYS_STATE_INIT, "pcrHandle unknown.");

    /* Initial invocation of SAPI to prepare the command buffer with parameters */
    r = Tss2_Sys_PCR_Event_Prepare(
...
Esys_PCR_Event_Finish(ESYS_CONTEXT *esysContext, TPML_DIGEST_VALUES **digests) {
    TSS2_RC r;
    LOG_TRACE("context=%p, digests=%p", esysContext, digests);
     ...
     */
    r = Tss2_Sys_PCR_Event_Complete(esysContext->sys, (digests != NULL) ? *digests : NULL);
