...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_NV_Extend
...
Esys_NV_Extend(ESYS_CONTEXT              *esysContext,
    ...
    r = Esys_NV_Extend_Async(esysContext, authHandle, nvIndex, shandle1, shandle2, shandle3, data);
...
Esys_NV_Extend_Async(ESYS_CONTEXT              *esysContext,
    ...
    r = Tss2_Sys_NV_Extend_Prepare(
        esysContext->sys, (authHandleNode == NULL) ? TPM2_RH_NULL : authHandleNode->rsrc.handle,
        (nvIndexNode == NULL) ? TPM2_RH_NULL : nvIndexNode->rsrc.handle, data);
