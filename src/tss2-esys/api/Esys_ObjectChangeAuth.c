...
#include "tss2_esys.h"       // for ESYS_CONTEXT, ESYS_TR, Esys_ObjectChang...
#include "tss2_sys.h"        // for Tss2_Sys_ExecuteAsync, TSS2L_SYS_AUTH_C...
#include "tss2_tpm2_types.h" // for TPM2B_AUTH, TPM2B_PRIVATE, TPM2B_PUBLIC
...
Esys_ObjectChangeAuth(ESYS_CONTEXT     *esysContext,
    ...
    r = Esys_ObjectChangeAuth_Async(esysContext, objectHandle, parentHandle, shandle1, shandle2,
                                    shandle3, newAuth);
...
Esys_ObjectChangeAuth_Async(ESYS_CONTEXT     *esysContext,
    ...
    TPM2B_AUTH             authCopy = { .size = 0, .buffer = {} };
    ...
    r = Tss2_Sys_ObjectChangeAuth_Prepare(
        esysContext->sys, objectHandleNode->rsrc.handle,
        (parentHandleNode == NULL) ? TPM2_RH_NULL : parentHandleNode->rsrc.handle, &authCopy);
...
Esys_ObjectChangeAuth_Finish(ESYS_CONTEXT *esysContext, TPM2B_PRIVATE **outPrivate) {
    TSS2_RC r;
    LOG_TRACE("context=%p, outPrivate=%p", esysContext, outPrivate);
